...
空数组默认推断为`any[]`,不过这个和**相关类型检查机制有关联**, `noImplicitAny` 为 `true` 时才会生效。当 `noImplicitAny` 为 `false` 时,空数组的类型推断为 `never[]`。
...
**接口**是面向对象的概念，因此它是定义对象结构的一种方式，它描述了对象的形状，即对象应该有哪些属性以及属性的类型。**接口主要用于声明对象的结构**
