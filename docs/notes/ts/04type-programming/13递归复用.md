...
type StringToUnion<S extends string> = S extends `${infer One}${infer Two}${infer Three}${infer Four}${infer Five}` ? One | Two | Three | Four | Five : never;
...
```typescript
type ReverseArr<T extends any[]> = T extends [infer One, infer Two, infer Three, infer Four, infer Five] ? [Five, Four, Three, Two, One] : never;
...
我们可以思考，之前我们讲过数组类型是不是可以获取长度，通过`T['length']`，那我们能不能把字符串类型转成数组类型呢？完全可以，通过infer推断和递归复用：
