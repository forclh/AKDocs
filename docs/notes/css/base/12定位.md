...
::: details 滚动子元素
当父元素设置了 `overflow: auto` 和 `position: relative` 时，它其实创造了两个东西：

1. 可视窗口：你眼睛能看到的那个固定大小的框（比如 200px 高）。
2. 滚动内容区域：这是一个可以无限延伸的画布，包含了所有的子元素。`position: absolute; top: 0`; 是相对于“滚动内容区域”的顶部，而不是“可视窗口”的顶部。
...
- 脱离文档流，不再占据原来的空间。
- 默认宽高为 `auto`，随内容收缩。
- 偏移相对于“最近的定位祖先”的填充盒原点。在**滚动容器**中，该原点位于内容的起始角 ，并会**随着内容的滚动而移动**。（无则相对于页面初始包含块）。
