...
上面的代码就是我们经常在 *ES5* 中所书写的代码，通过构造函数来模拟类，**实例方法挂在原型上面，静态方法就挂在构造函数**上。
...
可以看到，***ES6* 中的原型方法是不可被枚举**的，说明 *ES6* 对此也是做了特殊处理的。
...
类和构造函数的区别总结：
1. 类只能用new关键字创建实例，否则会报错；构造函数可以直接调用，返回undefined。
2. 类中的原型方法不可被枚举，构造函数则可以。
3. 类中的代码处于严格模式下。
4. 类原型上的方法不可以通过new来调用
