...
虽然变量的数据类型是不确定的，但是各种**运算符对数据类型是有要求**的。如果**运算符发现，运算子的类型与预期不符，就会自动转换类型**。比如，减法运算符预期左右两侧的运算子应该是数值，如果不是，就会自动将它们转为数值。
...
下面两种写法，有时也用于**将一个表达式转为布尔值**。它们内部调用的也是`Boolean()`函数。
...
JavaScript 遇到**预期为字符串**的地方，就会将非字符串的值自动转为字符串。具体规则是，先将复合类型的值转为原始类型的值，再将原始类型的值转为字符串。
