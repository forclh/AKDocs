...
属性描述符是 *ECMAScript* 5 新增的语法，它其实就是一个**内部对象，用来描述对象的属性的特性**。
...
*configurable* 可以**禁止修改属性描述符**，当其值为 *false* 时，*value、writable、enumerable* 和 *configurable* 禁止修改，同时禁止删除属性。
...
- *Object.seal*：阻止为对象添加新的属性，同时也无法删除旧属性。等价于属性描述符的 *configurable* 属性设为 *false*。注意，该方法**不影响修改某个属性的值**。
