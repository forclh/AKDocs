...
2. **动态渲染**：基于Vue的动态组件机制，实现组件的即时渲染和展示
...
核心使用Vue的 `<component :is>` 语法实现动态组件渲染：
...
通过 `componentMap` 建立组件名称与实际Vue组件的映射：
...
使用Vue的 `provide/inject` 机制实现跨层级的状态更新：
...
由于使用了Vue的响应式系统，当编辑器中的属性发生变化时，画布中的组件会自动重新渲染：
...
使用 `markRaw()` 避免Vue对组件实例进行深度响应式处理：
