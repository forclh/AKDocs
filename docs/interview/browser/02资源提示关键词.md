...
- `async`：下载与 HTML 解析并行；下载完成即执行，执行会阻塞解析；不保证顺序，适合独立脚本。
- `defer`：下载与解析并行；在解析完成、`DOMContentLoaded` 前按顺序执行；适合有依赖关系的脚本。
...
_preload_ 顾名思义就是一种预加载的方式，它通过声明<span style="color:red">**向浏览器声明一个需要提前加载的资源，当资源真正被使用的时候立即执行，就无需等待网络的消耗**</span>。
...
_prefetch_ 是一种<span style="color:red">**利用浏览器的空闲时间加载页面将来可能用到的资源的一种机制，通常可以用于加载非首页的其他页面所需要的资源，以便加快后续页面的首屏速度。**</span>
...
_DNS prefetching_ 允许<span style="color:red">**浏览器在用户浏览时在后台对页面执行 _DNS_ 查找**</span>。这最大限度地减少了延迟，**因为一旦用户单击链接就已经进行了 _DNS_ 查找。**
...
不同之处在于 <span style="color:red">**_prerender_ 实际上是在后台渲染整个页面，而 _prefetch_ 只是在后台下载资源**</span>。
...
_preconnect_ 指令允许<span style="color:red">**浏览器在 _HTTP_ 请求实际发送到服务器之前设置早期连接**</span>。
